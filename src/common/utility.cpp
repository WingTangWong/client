    ...
    const QString &fn, const QDateTime &dt, const QString &user)
{
    ...
    QString conflictMarker = QStringLiteral(" (Conflict ");
    if (!user.isEmpty()) {
...
bool Utility::isConflictFile(const QString &name)
{
    ...
    if (bname.contains(QStringLiteral("(Conflict")))
        return true;
...
QByteArray Utility::conflictFileBaseName(const QByteArray &conflictName)
{
    ...
    // A single space before "(Conflict" is considered part of the tag
    auto startNew = conflictName.lastIndexOf("(Conflict");
