...
bool expectAndWipeConflict(FileModifier &local, FileInfo state, const QString path)
{
    ...
    for (const auto &item : base->children) {
        if (item.name.startsWith(pathComponents.fileName()) && item.name.contains("(Conflict")) {
...
private slots:
    ...
    void testUploadAfterDownload()
    {
        ...
        QVERIFY(conflictMap[a1FileId].contains(QString("(Conflict %1 ").arg(fakeFolder.syncEngine().account()->davDisplayName())));
    ...
    void testSeparateUpload()
    {
        ...
        auto a1FileId = fakeFolder.remoteModifier().find("A/a1")->fileId;
        QString conflictName = QLatin1String("A/a1 (Conflict me 1234)");
    ...
    void testDownloadingConflictFile()
    {
        ...
        fakeFolder.remoteModifier().insert("A/a1 (Conflict 1234)");
        QVERIFY(fakeFolder.syncOnce());
        QCOMPARE(fakeFolder.currentLocalState(), fakeFolder.currentRemoteState());
        auto conflictRecord = fakeFolder.syncJournal().conflictRecord("A/a1 (Conflict 1234)");
    ...
    void testConflictFileBaseName_data()
    {
        ...
        QTest::newRow("newmatch1")
            << "a/b/foo (Conflict 123).txt"
            << "a/b/foo.txt";
        QTest::newRow("newmatch2")
            << "a/b/foo (Conflict foo 123).txt"
            << "a/b/foo.txt";

        QTest::newRow("newmatch3")
            << "a/b/foo (Conflict 123)"
            << "a/b/foo";
        QTest::newRow("newmatch4")
            << "a/b/foo (Conflict foo 123)"
            << "a/b/foo";

        QTest::newRow("newmatch5")
            << "a/b/foo (Conflict foo 123) bla"
            << "a/b/foo bla";

        QTest::newRow("newmatch6")
            << "a/b/foo (Conflict foo.bar 123)"
            << "a/b/foo";
        ...
        QTest::newRow("double3")
            << "a/b/foo (Conflict 123) (Conflict 456).txt"
            << "a/b/foo (Conflict 123).txt";
        QTest::newRow("double4")
            << "a/b/foo (Conflict 123)_conflict-456.txt"
            << "a/b/foo (Conflict 123).txt";
        QTest::newRow("double5")
            << "a/b/foo_conflict-123 (Conflict 456).txt"
            << "a/b/foo_conflict-123.txt";
    }
    ...
    void testTypeConflictWithMove()
    {
        ...
        QVERIFY(conflicts[0].contains("A (Conflict"));
        QVERIFY(conflicts[1].contains("B (Conflict"));
